...
const AnimatedHamburgerButton: React.FC<{ onClick: () => void; active: boolean }> = ({ onClick, active }) => {
  ...
  return (
    <MotionConfig transition={{ duration: 0.4, ease: "easeInOut" }}>
      ...
      >
        <motion.span className="block w-7 h-0.5 mb-1 bg-black" variants={VARIANTS.top} />
        <motion.span className="block w-7 h-0.5 mb-1 bg-black" variants={VARIANTS.middle} />
        <motion.span className="block w-7 h-0.5 bg-black" variants={VARIANTS.bottom} />
      </motion.button>
    </MotionConfig>
  );
};
  ...
  return (
    <>
      ...
      ">
        ...
        {/* Botão Hamburguer (MOBILE SOMENTE) */}
        <div className="block sm:hidden absolute left-4">
        ...
        {/* Links */}
        <ul className={`
          flex gap-8 list-none text-black font-medium text-lg
          sm:flex-col sm:w-full sm:text-center sm:mt-16
          sm:overflow-hidden sm:transition-[max-height] sm:duration-300
          ${menuOpen ? "sm:max-h-64" : "sm:max-h-0"}
        `}>
