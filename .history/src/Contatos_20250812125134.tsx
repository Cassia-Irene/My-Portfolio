...
const SocialButton = styled.a< { borderGradient?: string; centerColor?: string }>`
  width: 56px;
  height: 56px;
  border-radius: 50%;
  display: flex;
  ...
  &::after {
    ...
    /* Centro transparente antes do hover */
  &::before {
    content: "";
    position: absolute;
    inset: 2px; /* cria o espaço entre borda e fundo */
    border-radius: 50%;
    background: transparent;
    transition: all 0.3s ease;
    z-index: 1;
  ...
  &:hover {
    &::after {
      ...
      background: ${({ borderGradient }) => borderGradient || "transparent"};
      padding: 2px; /* simula o espaçamento entre borda e centro */
      -webkit-mask: 
        radial-gradient(circle, #000 calc(100% - 2px), transparent calc(100% - 1px));
      mask: 
        radial-gradient(circle, #000 calc(100% - 2px), transparent calc(100% - 1px));
      box-shadow: 0 0 10px ${({ borderGradient }) => borderGradient || "#fff"};
    }

    &::before {
      background: ${({ centerColor }) => centerColor || "transparent"};
    }
  }
    ...
    <SectionContatos id="section-04">
      ...
      <SocialWrapper>
        ...
        <SocialButton
          ...
          borderBg="linear-gradient(90deg, #4285F4 0%, #C5221F 25%, #EA4335 50%, #FBBC04 75%, #34A853 100%)"
          centerBg="#C5221F"
        >
